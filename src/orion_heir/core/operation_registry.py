...
class CKKSMulScalarHandler(BaseOperationHandler):
    ...
    * **Genuine float scalar**: encode at the ciphertext's plaintext scale,
    ...
    def handle(
        ...
        dense_attr = DenseIntOrFPElementsAttr.create_dense_float(tensor_type, [scalar_val] * slots)
        ...
        if is_integer:
            ...
            pt_encoding = InverseCanonicalEncodingAttr([IntegerAttr(0, IntegerType(32))])
        ...
        if is_integer:
            mul_op = MulPlainOp(operands=[current_value, plaintext], result_types=[ct_ty])
            block.add_op(mul_op)
            ...
            rescaled_type = type_builder.create_rescaled_type(mul_result.type, original_scale)
            rescale_op = RescaleOp(
                operands=[mul_result],
                result_types=[rescaled_type],
                properties={"to_ring": type_builder.get_next_modulus_ring(mul_result.type)},
            )
            block.add_op(rescale_op)
